...
class DeviceGroupQuery(
    ...
    context: ActorContext<DeviceGroupQueryCommand>) : AbstractBehavior<DeviceGroupQueryCommand>(context) {
    ...
    init {
        ...
        val respondTemperatureAdapter = context.messageAdapter(RespondTemperature::class.java) { respond -> WrappedRespondTemperature(respond) }
    ...
    companion object {
         ...
         * 创建一个自带延时功能的 Query
         */
        fun create(deviceIdToActor: Map<String,ActorRef<DeviceCommand>>,
                   requestId: Long,
                   requester: ActorRef<RespondAllTemperature>,
                   timeout: Duration): Behavior<DeviceGroupQueryCommand> {
            return Behaviors.setup {
                context -> Behaviors.withTimers { scheduler -> DeviceGroupQuery(deviceIdToActor, requestId, requester, timeout, scheduler, context) }
            }
        }
